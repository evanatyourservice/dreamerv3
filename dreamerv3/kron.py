...
def scale_by_kron(
    ...
    **kwargs,
    ...
    def init_fn(params, return_partition_specs_only=False):
        ...
        state = (jnp.zeros([], jnp.int32), mu, Qs, jnp.zeros([], jnp.int32))
        ...
        pprint(
            jax.tree.map(
                ...
                dict(
                    count=state[0],
                    mu=state[1],
                    Qs_preconditioners=state[2],
                    update_counter=state[3],
                ),
            ),
            width=100,
        )
    ...
    def update_fn(updates: base.Updates, state: tuple, params: base.Params = None):
        del params
        state = dict(
            count=state[0],
            mu=state[1],
        ...
        state = (count_inc, mu, Qs, update_counter_inc)
...
def _get_preconditioner_types(
    ...
    elif mem_save_mode == "smart_one_diag":
